        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <div className="space-y-4">
            <ToolField label="Age" htmlFor="crs-age">
              <ToolInput id="crs-age" type="number" min={18} max={50} value={form.age} onChange={(e) => setForm((f) => ({ ...f, age: clamp(Number(e.target.value)) }))} />
            </ToolField>
          ...
          <div className="space-y-4">
            <ToolCheckboxCard label="Applying with a spouse / partner" hint="Toggle to include spouse factors" checked={form.hasSpouse} onChange={setBool("hasSpouse")} />
            {form.hasSpouse && (
            ...
            <ToolCheckboxCard label="Sibling living in Canada (citizen/PR)" checked={form.sibling} onChange={setBool("sibling")} />
          </div>
        </div>
        ...
        <ToolResultCard className="mt-7">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="tool-result__score mt-1 text-5xl font-extrabold tracking-tight sm:text-6xl">{score}</p>
