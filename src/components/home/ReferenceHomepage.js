...
export default function ReferenceHomepage({ page, heroData }) {
  ...
  return (
    <div className="cmg-template-home" data-concept="nocturne">
        ...
        <div className="hero-layout">
          <div className="hero-copy reveal in">
            ...
            <div className="hero-trust">
              <span className="trust-chip"><ShieldCheck width={16} height={16} aria-hidden="true" /> {trustItems[0]}</span>
          ...
          <div className="hero-visual reveal in" aria-label="High-stakes files — sharper strategy">
            <article className="visual-card">
              <div className="mini-badges">
                <span className="mini-badge"><ShieldCheck width={15} height={15} aria-hidden="true" /> CICC-regulated</span>
      ...
      ].map(([initials, name, route, quote]) => <figure className="quote-card reveal" key={name}><div className="stars flex items-center gap-0.5" aria-label="5 out of 5 stars">{[...Array(5)].map((_, i) => <Star key={i} className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />)}<span className="ml-1 text-xs font-bold text-amber-500">5.0</span></div><blockquote>{quote}</blockquote><figcaption><span className="avatar">{initials}</span><span><strong>{name}</strong><small>{route}</small></span></figcaption></figure>)}</div></div></section>
