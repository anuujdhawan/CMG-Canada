...
export default function ReferenceHomepage({ page, heroData }) {
  ...
  const pageDataBullets = heroData?.badges || [];
  ...
  const fallbackTrustItems = ["CICC-regulated", "Canada-wide support", "No-obligation first step"];
  const trustItems = fallbackTrustItems.map((fallback, index) => pageDataBullets[index] || fallback);
    ...
    <div className="cmg-template-home" data-concept="nocturne">
        ...
        <div className="hero-layout">
          <div className="hero-copy reveal in">
            ...
            <div className="hero-actions">
              ...
              <TemplateLink path="/assessment/free-canada-immigration-assessment" className="btn btn-secondary">Free Assessment <ArrowUpRight width={18} height={18} aria-hidden="true" /></TemplateLink>
            </div>
            <div className="hero-trust">
              <Link href="https://register.college-ic.ca/Public-Register-EN/Licensee/Profile.aspx?ID=18715" target="_blank" rel="noopener noreferrer" className="trust-chip"><ShieldCheck width={16} height={16} aria-hidden="true" /> {trustItems[0]}</Link>
              <span className="trust-chip"><MapPin width={16} height={16} aria-hidden="true" /> {trustItems[1]}</span>
              <span className="trust-chip"><Clock3 width={16} height={16} aria-hidden="true" /> {trustItems[2]}</span>
            </div>
          </div>
          <div className="hero-visual reveal in" aria-label="High-stakes files — sharper strategy">
            <article className="visual-card">
              <div className="mini-badges">
                <Link href="https://register.college-ic.ca/Public-Register-EN/Licensee/Profile.aspx?ID=18715" target="_blank" rel="noopener noreferrer" className="mini-badge"><ShieldCheck width={15} height={15} aria-hidden="true" /> CICC-regulated</Link>
                <span className="mini-badge"><MapPin width={15} height={15} aria-hidden="true" /> Canada-wide</span>
              </div>
              <h2>A calmer start to a complicated journey.</h2>
              <p>When the file is consequential, clarity, evidence and ownership matter at every stage.</p>
              <div className="visual-stats">
                <div><strong data-count="15">15<span>+</span></strong><small>years of team experience</small></div>
                <div><CircleCheckBig width={24} height={24} aria-hidden="true" /><small>Clear next steps,<br />not guesswork</small></div>
              </div>
            </article>
          </div>
        </div>
      ...
      <section className="section" id="pathways">
        <div className="section-inner">
          <SectionHeading eyebrow="Start with the right route" title="Choose the pathway that feels like yours" lead="The best immigration plan starts with the right question. Pick a goal and we will take you to the deeper guide, tools and next steps." />
      ...
      <section className="section"><div className="section-inner"><SectionHeading eyebrow="Real client reviews" title="Trusted by clients worldwide — 5.0 rated" lead="Honest guidance, quick replies and approvals that actually happen. Hear from clients across India, Pakistan and Europe who trusted CMG with their Canada journey." /><div className="testimonial-grid">{[
        ["PS", "Priya Sharma", "Mumbai, India · Express Entry", "“CMG turned my Express Entry confusion into a clear, step-by-step plan. They boosted my CRS by 38 points and I got my ITA in just 3 weeks. Transparent, responsive and genuinely professional.”"],
        ["AH", "Ahmed Hassan", "Lahore, Pakistan · Study Permit", "“After my first refusal I was hopeless. CMG rewrote my SOP, fixed my bank statements and got my study permit approved in 21 days. I recommend them to every student in Pakistan.”"],
        ["ES", "Elena Schneider", "Munich, Germany · LMIA & Work Permit", "“Hiring through LMIA from Germany felt impossible until CMG stepped in. Every compliance detail was perfect and our engineer landed in Toronto right on schedule. Absolutely 5 stars.”"]
      ].map(([initials, name, route, quote]) => <figure className="quote-card reveal" key={name}><div className="stars flex items-center gap-0.5" aria-label="5 out of 5 stars">{[...Array(5)].map((_, i) => <Star key={i} className="template-star h-3.5 w-3.5" />)}<span className="template-star ml-1 text-xs font-bold">5.0</span></div><blockquote>{quote}</blockquote><figcaption><span className="avatar">{initials}</span><span><strong>{name}</strong><small>{route}</small></span></figcaption></figure>)}</div></div></section>
