    ...
    <div className="cmg-template-home" data-concept="nocturne">
      ...
      <section className="section alt">
        <div className="section-inner guide-layout">
          <div className="guide-image reveal"><Image src="/images/hero/toronto-skyline-autumn-park.jpg" alt="Toronto skyline beyond an autumn park" fill sizes="(max-width: 880px) 100vw, 40vw" /></div>
      ...
      <section className="section"><div className="section-inner"><SectionHeading eyebrow="Illustrative client perspectives" title="Clarity is part of the service" lead="These role-based examples show the kind of preparation a focused consultation can create. They are illustrative and are not presented as named client reviews." /><div className="testimonial-grid">{[["EE", "Illustrative Express Entry profile", "Profile planning · Canada", "“The review helped me separate the facts that mattered from the noise. I left with a document list and a realistic order of operations."], ["SP", "Illustrative study permit profile", "Study planning · Canada", "“The study-plan discussion made the weak points visible before submission. The next steps felt practical instead of overwhelming."], ["CE", "Illustrative employer profile", "Workforce planning · Canada", "“Our hiring question became a clear sequence of employer-side checks, evidence and deadlines that the team could actually manage." ]].map(([initials, name, route, quote]) => <figure className="quote-card reveal" key={name}><div className="stars" aria-label="Illustrative example">—</div><blockquote>{quote}</blockquote><figcaption><span className="avatar">{initials}</span><span><strong>{name}</strong><small>{route}</small></span></figcaption></figure>)}</div></div></section>
